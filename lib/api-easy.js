    ...
    _request: function (/* method [uri, data, params] */) {
      ...
      if (data) {
        if (this.outgoing.headers['Content-Type'] == 'application/x-www-form-urlencoded') {
          outgoing.body = objectToQueryString(data);
        } 
        else {
...
function objectToQueryString(object, prefix) {
  //http://stackoverflow.com/questions/7030087/encode-client-request-params-in-node-js
  var result = '',
      key = '',
      postfix = '&',
      i;
  ...
  for (i in object) {
    ...
    key = ( ! prefix) ? qs.escape(i) : prefix + '[' + qs.escape(i) + ']';
    // String pass as is...
    if (typeof(object[i]) == 'string') {
      result += key + '=' + qs.escape(object[i]) + postfix;
      continue;
    }
    // objectects and arrays pass depper
    if (typeof(object[i]) == 'object' || typeof(object[i]) == 'array') {
      result += objectToQueryString(object[i], key) + postfix;
      continue;
    }
    // Other passed stringified
    if (object[i].toString) {
      result += key + '=' + qs.escape(object[i].toString()) + postfix;
      continue;
    }
  }
